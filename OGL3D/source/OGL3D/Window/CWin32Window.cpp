...
OWindow::OWindow()
{
	...
	// Load a .ico from disk. LR_DEFAULTSIZE picks a good size from the .ico.
	HICON hBig = (HICON)LoadImageW(nullptr, L"assets\\icon.ico", IMAGE_ICON, 0, 0,
		LR_LOADFROMFILE | LR_DEFAULTSIZE);
	HICON hSmall = (HICON)LoadImageW(nullptr, L"assets\\icon.ico", IMAGE_ICON, 16, 16,
		LR_LOADFROMFILE);

	// Apply to the window (runtime)
	if (hBig)   SendMessageW((HWND)m_handle, WM_SETICON, ICON_BIG, (LPARAM)hBig);
	if (hSmall) SendMessageW((HWND)m_handle, WM_SETICON, ICON_SMALL, (LPARAM)hSmall);
	...
	int pixelFormat = 0;
	UINT numFormats = 0;
	wglChoosePixelFormatARB(hDC, pixelFormatAtributes, nullptr, 1, &pixelFormat, &numFormats);
	assert(numFormats);
